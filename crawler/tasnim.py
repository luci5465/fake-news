...
def extract_content(soup, url):
    title = ""
    selectors_title = ["h1.title", "h1.news-title", "h1", "meta[property='og:title']"]
    ...
    if title:
        title = title.split("|")[0].split("-")[0].strip().replace("\u200c", " ")
    else:
        return None 

    content = ""
    selectors_content = ["div.story", "div.news-text", "div.body-content", "article"]
    content_soup = None
    ...
    date = ""
    selectors_date = ["li.time", "span.time", "div.news-time", "meta[property='article:published_time']"]
